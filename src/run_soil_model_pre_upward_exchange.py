...
from empirical Hero Dirt parameters:
        effective tread Ksat fraction
        shallow/deep transfer scalings
        ET coefficients
        score thresholds

Those empirical parameters remain provisional.
"""
...
#
# Spatially modified by Ksat and texture.
...
# ------------------------------------------------------------
# Ksat normalized to 5-95 percentile range
# ------------------------------------------------------------
...
#     0.5 - 1.5
#
# Hard bounds prevent extreme extrapolation.
# ------------------------------------------------------------
...
# Initially tread and shallow layers share the same
# upper-5-cm state.
...
def advance_interval(
    ...
    # Rain gets 5-minute integration.
    # Dry periods only need <=1-hour integration.
...
# ANALYSIS MET PROXY
#
# Still a known limitation:
# first NWS forecast field is being used as the atmospheric
# drying proxy during the observed MRMS analysis period.
# ============================================================
...
if len(
    ...
    # MRMS field represents the preceding hour.
...
print(
    f"  tread theta:      "
    f"{masked_mean(current_theta05*0 + tread):.4f}"
)
